...
function fetchWeatherRSS() {
  ...
  cities.forEach(function(city) {
    ...
    try {
      ...
      // Safety check: Don't crash on one city failing
      ...
      for (let i = 0; i < entries.length; i++) {
          ...
          const title = entry.getChild("title", root.getNamespace()).getText();
          const summary = entry.getChild("summary", root.getNamespace()).getText();
          
          let category = "Unknown";
          const catEl = entry.getChild("category", root.getNamespace());
          ...
          // B. WARNINGS (SEVERITY TRACKING)
              ...
              if (!title.includes("No watches or warnings")) {
                  ...
                  let severity = 1;
                  if (lowerTitle.includes("warning")) severity = 3;
                  else if (lowerTitle.includes("watch")) severity = 2;
          ...
          // C. FORECAST
              ...
              const fParts = title.split(":");
              if (fParts.length > 1 && forecastData.length < 7) {
                  const fDay = fParts[0].trim();
                  let fWind = "Light";
